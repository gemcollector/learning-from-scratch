...
class PPO:
    ...
    def __init__(
        ...
        # self.actor_critic.obs_enc.backbone.to(self.device)
        ...
        if self.DA:
            ...
            self.aug = RandomShiftsAug(pad=10)
        self.drac = drac
        if self.drac:
            print('================We will use drac loss====================')
    ...
    def log(
        ...
        f = open(f'{self.log_dir}/success_rate.txt', 'a+')
        lines = f"{it}, {self.tot_timesteps}, {mean_success_rate}"
        f.writelines(lines)
        f.write('\n')
    ...
    def update(self, cur_iter, max_iter):
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for indices in batch:
                ...
                if self.is_image and self.DA and self.drac:
                    obs_aug_batch = self.aug(obs_batch)
                    _, obs_batch = self.actor_critic.obs_enc(obs_batch)
                    _, obs_aug_batch = self.actor_critic.obs_enc(obs_aug_batch)
                if self.is_image and not self.DA and not self.drac:
                    _, obs_batch = self.actor_critic.obs_enc(obs_batch)
                if self.is_image and self.DA and not self.drac:
                    obs_batch = self.aug(obs_batch)
                    _, obs_batch = self.actor_critic.obs_enc(obs_batch)
                ...
                if self.drac:
                    aug_actions_log_prob_batch, aug_entropy_batch, aug_value_batch, aug_mu_batch, aug_sigma_batch = \
                        self.actor_critic(obs_aug_batch, states_batch, actions_batch)
                    # value_batch = (value_batch + aug_value_batch) / 2
                    value_target = copy.deepcopy(value_batch.data)
                    aug_value_loss = (value_target - aug_value_batch).pow(2).mean()
