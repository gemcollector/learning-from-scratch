...
class KukaMove(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        max_agg_bodies = num_kuka_bodies + num_table_bodies + num_object_bodies + num_goal_bodies
        max_agg_shapes = num_kuka_shapes + num_table_shapes + num_object_shapes + num_goal_shapes
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:], self.successes[:] = compute_kuka_reward(
            ...
            self.object_pos, self.goal_pos, self.object_z_init,
    ...
    def reset(self, env_ids):
        ...
        multi_env_ids_int32 = torch.cat([object_multi_env_ids_int32, goal_multi_env_ids_int32])
...
def compute_kuka_reward(
    ...
    object_pos: Tensor, goal_pos: Tensor, object_z_init: float,
    ...
    successes = torch.where(og_d <= 0.0675, torch.ones_like(successes) + successes, s)
