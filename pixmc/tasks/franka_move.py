...
class FrankaMove(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless, normalize):
        ...
        # Object to goal
        self.to_goal = torch.zeros((self.num_envs, 3), dtype=torch.float, device=self.device)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        goal_start_pose.p = gymapi.Vec3(0.5, 0.0, 0.7)
        ...
        max_agg_bodies = num_franka_bodies + num_table_bodies + num_object_bodies + num_goal_bodies
        max_agg_shapes = num_franka_shapes + num_table_shapes + num_object_shapes + num_goal_shapes
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:], self.successes[:] = compute_franka_reward(
            ...
            self.lfinger_grasp_pos, self.rfinger_grasp_pos, self.object_pos, self.goal_pos,
    ...
    def reset(self, env_ids):
        ...
        # Combined object and goal multi env ids
        object_goal_multi_env_ids_int32 = torch.cat([object_multi_env_ids_int32, goal_multi_env_ids_int32])
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            ...
            gymtorch.unwrap_tensor(object_goal_multi_env_ids_int32),
            len(object_goal_multi_env_ids_int32)
        )
    ...
    def compute_task_state(self):
        ...
        self.to_goal[:] = self.goal_pos - self.object_pos
...
def compute_franka_reward(
    ...
    # Goal reached TODO
    successes = torch.where(og_d <= 0.04, torch.ones_like(successes), successes)
